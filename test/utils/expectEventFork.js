...
function contains (args, key, value) {
    ...
    if (value === null) {
        ...
        if (typeof value === 'object') {
            const obj = {};
            const newValue = Object.assign({}, value);//expected value
            Object.keys(newValue).forEach((currentKey) => {
                obj[currentKey] = args[key][currentKey];
                if (typeof obj[currentKey] === 'string') {
                    obj[currentKey] = `${obj[currentKey].toLowerCase()}`;
                }
                if (typeof newValue[currentKey] === 'string') {
                    newValue[currentKey] = `${newValue[currentKey].toLowerCase()}`;
                }
                if (typeof newValue[currentKey] === 'number') {
                    newValue[currentKey] = `${newValue[currentKey]}`;
                }
            });
            const diff = _.differenceWith(obj, newValue, _.isEqual);
            // console.log(diff);
            expect(_.isEqual(obj, newValue)).to.be.equal(true,
              `expected event argument '${key}' to have value ${JSON.stringify(newValue)} but got ${JSON.stringify(obj)}, diff ${diff}`);

        } else {
            if (typeof args[key] === "string" && typeof value === "string") {
                expect(args[key].toLowerCase()).to.be.deep.equal(value.toLowerCase(),
                  `expected event argument '${key}' to have value ${value} but got ${args[key]}`);
            }
            else if (typeof args[key] === "boolean" && typeof value === "boolean") {
