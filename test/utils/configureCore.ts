...
import SignatureVerifierJson from '../../artifacts/@debridge-finance/debridge-contracts-v1/contracts/transfers/SignatureVerifier.sol/SignatureVerifier.json';
...
export const createCoreContract = async (deployer: SignerWithAddress): Promise<{ deBridgeGateETH: MockDeBridgeGate, deBridgeGateBSC: MockDeBridgeGate, mockWeth: MockWeth }> => {
    ...
    await deBridgeGateETH.updateGlobalFee(deployInitParamsETH.globalFixedNativeFee, GLOBAL_TRANSFER_FEE_BPS);
    ...
    await deBridgeGateBSC.updateGlobalFee(deployInitParamsBSC.globalFixedNativeFee, GLOBAL_TRANSFER_FEE_BPS);
